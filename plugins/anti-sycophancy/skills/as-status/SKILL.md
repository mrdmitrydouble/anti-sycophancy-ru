...
1. Read the file `.claude/.as-mode`.
   - `auto` → **ON (auto)** — relational critic chosen per turn. `on` (a legacy value) means the same.
   - `couple` → **ON (couple/group)** — relational critic is as-guardian.
   - `solo` → **ON (solo)** — relational critic is as-solo-guardian.
   - `off`, empty, missing, or any other value → **OFF** (for an unrecognized value, add that the flag file holds an unknown value).
