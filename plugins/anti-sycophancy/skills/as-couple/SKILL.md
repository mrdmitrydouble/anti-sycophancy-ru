...
Follow the full protocol (see the `as-orchestrator` agent or the `as-on` skill): as-critic-blind always; as-guardian when a second side is present; as-safety only on a deterministic gate hit; one rewrite pass. Pass drafts and quotes inline between `"""` fences (the input guard refuses file paths) and use only verdicts the verification note marks VALID.
